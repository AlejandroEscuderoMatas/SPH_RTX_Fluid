...
void HelloVulkan::updateUniformBuffer(const VkCommandBuffer& cmdBuf)
{
  ...
  // Change the usage stage flag to include ray tracing
...
void HelloVulkan::destroyResources()
{
  ...
  // Destroy resources of TLAS
  m_rtBuilder.destroy();
  // Destroy Descriptor Objects of RTX
  vkDestroyDescriptorPool(m_device, m_rtDescPool, nullptr);
  vkDestroyDescriptorSetLayout(m_device, m_rtDescSetLayout, nullptr);
  // Destroy resources used to create the pipeline
  vkDestroyPipeline(m_device, m_rtPipeline, nullptr);
  vkDestroyPipelineLayout(m_device, m_rtPipelineLayout, nullptr);
  // Destroy SBT resources
...
void HelloVulkan::initRayTracing()
{
  ...
  //Initialize the helper for the Acceleration Structure
...
void HelloVulkan::createBottomLevelAS()  // Creates an array of BLASes
{
...
void HelloVulkan::createRtPipeline()
{
  ...
  // closest hit shader - Hit group 1, Payload 0
  group.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR;  //if the geometry were not triangles,
  group.generalShader = VK_SHADER_UNUSED_KHR;  // we would have set the type to VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_KHR
  group.closestHitShader = eClosestHit;  // , and would have to define an intersection shader.
  group.anyHitShader     = eAnyHit;
  m_rtShaderGroups.push_back(group);

  // closest hit shader - Hit gruop 2, Payload 1
  group.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR;  //if the geometry were not triangles,
  group.generalShader = VK_SHADER_UNUSED_KHR;  // we would have set the type to VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_KHR
  group.closestHitShader = VK_SHADER_UNUSED_KHR;  // , and would have to define an intersection shader.
  ...
  // The pipeline uses two descriptor sets: set=0 is specific to the ray tracing pipeline (TLAS and output image),
  ...
  rayPipelineInfo.maxPipelineRayRecursionDepth = 2;  // Ray depth: How many rays can be traced recursively. 1 for rgen and another one for chit
...
void HelloVulkan::createRtShaderBindingTable()
{
  ...
  // The following sets the stride and size for each group. With the exception of RayGen, the stride will be the size of the handle aligned to
  // the shaderGroupHandleAlignment. And the size of each group, is the number of elements in the group aligned to the shaderGroupBaseAlignment
  ...
  // The following will allocate the buffer that will hold the handle data. Note that the SBT buffer needs the
  // VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR flag. In order to trace rays we will also need the address of the SBT,
  // which requires the VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT flag.
  ...
  // This lambda function will return the pointer to the previously retrieved handle. We will use this function to copy the data from
  // the handle into the SBT buffer.
